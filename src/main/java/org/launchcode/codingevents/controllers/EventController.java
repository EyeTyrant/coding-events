...
//  private static List<Event> events = new ArrayList<>(); // Now stored in EventData class
  ...
  @Autowired
  private TagRepository tagRepository;  // Creates instance of TagRepository to hold the tags created
...
//    EventData.getById(eventId).setDescription(description); // EventData class no longer needed, data is now stored in database and handled with EventRepository class.
  ...
  // responds to /events/add-tag?eventId=69
  @GetMapping("add-tag")
  ...
  @PostMapping("add-tag")
  public String processAddTagFrom(@ModelAttribute @Valid EventTagDTO eventTag, Errors err, Model modl) {
